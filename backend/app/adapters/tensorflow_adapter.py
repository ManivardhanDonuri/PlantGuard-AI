...
# Mock TensorFlow classes for when TensorFlow is not available
class MockModel:
    def __init__(self, num_classes=30):
        self.num_classes = num_classes
        self.input_shape = (None, 224, 224, 3)
        self.output_shape = (None, num_classes)
        self._params = 1000000
    ...
    def predict(self, x, verbose=0):
        ...
        for i in range(batch_size):
            ...
            if green_channel > 0.4:
                healthy_indices = [3, 6, 10, 13, 17]
                for idx in healthy_indices:
                    if idx < self.num_classes:
                        probs[idx] *= 2.0
            
            if red_channel > green_channel:
                disease_indices = list(range(self.num_classes))
                for idx in [3, 6, 10, 13, 17]:
                    if idx in disease_indices:
                        disease_indices.remove(idx)
...
# Try to import TensorFlow, fall back to mock
try:
    ...
    tf = type('MockTF', (), {'keras': MockKeras()})()
    keras = tf.keras
    TF_AVAILABLE = False
